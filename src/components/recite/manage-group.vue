            ...
            <v-flex grow>
              <v-list three-line>
                <v-list-tile>
                  ...
                  <v-list-tile-content>
                    <v-list-tile-title>{{myGroupsData.name}}</v-list-tile-title>
                    <v-list-tile-sub-title>{{myGroupsData.country}}</v-list-tile-sub-title>
                  </v-list-tile-content>
                </v-list-tile>
              </v-list>
            </v-flex>
...
<v-flex>
  <div align="left" class="mx-2">
        <v-btn @click="showEditPortal=false" color="error">close</v-btn>
      </div>
      </v-flex>
      </v-layout>
      </div>
...
export default {
  ...
  methods: {
    ...
    create() {
      firebase.firestore().collection("groups").doc(this.name).collection('admin').doc(firebase.auth().currentUser.uid).set({
        name: this.name,
        country: this.select,
        url: this.photoURL,
        leaders: []
    ...
    update(mode, email) {
      ...
      if (mode === "add") {
        db.collection("groups").doc(this.userClaims.owner).collection('admin').doc(firebase.auth().currentUser.uid).update({
  ...
  watch: {
    ...
    showEditPortal: {
     ...
     handler(showEditPortal) {
       if(showEditPortal) {
       this.$bind('myGroupsData', db.collection("groups").doc(this.userClaims.owner).collection('admin').doc(firebase.auth().currentUser.uid))
     } else {
