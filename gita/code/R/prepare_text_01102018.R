  ...
  mywords[7492, c("verse", "foot", "chapter", "chapter_verse")] <- c(551, 4, 14, 27)  # for some reason only this was off!
  ...
  # store the words separately
  mytemp1 <- mywords %>% select(verse, slp1, english, foot) %>% nest(-verse)
  mytemp2 <- mywords %>% select(verse, chapter) %>% distinct()
  mytemp3 <- mywords %>% select(verse, chapter_verse) %>% distinct() 
  mywords_df <- mytemp1 %>%
    full_join(mytemp2) %>%
    full_join(mytemp3) %>%
    select(chapter, chapter_verse, data)
    
  mywords_json <- mywords_df %>% 
    toJSON(dataframe = "columns", pretty = TRUE)
  
  mywords_json %>% 
    write_lines("./data/export/gitabywords_slp1.json")
  
  # store the words plus foot together
  # mytemp1 <- mywords %>% select(verse, slp1, foot) %>% nest(-verse)
  mywords_json <- mymain_slp1_slp1 %>% 
    bind_cols(mywords_df %>% select(data)) %>%
    toJSON(dataframe = "columns", pretty = TRUE)
  ...
  mytemp1 <- mywords %>% select(verse, sanskrit, english, foot) %>% nest(-verse)
  mywords_json <- mymain_slp1_dev %>% 
    bind_cols(mytemp1 %>% select(data)) %>%
    toJSON(dataframe = "columns", pretty = TRUE)
